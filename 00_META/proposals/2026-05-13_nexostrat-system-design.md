...
| ADR-020 | Independent bot codebase | **Superseded by ADR-039** (2026-05-21) | 2026-05-11. Superseded after Session B of the Brain Bot Platform audit: Nexostrat bot accepted as tenant in the central Brain Bot Hub with strict isolation + separate Telegram token + separate OpenAI API key. See `00_GOVERNANCE/adr/ADR-039-bot-tenant-in-brain-hub.md`. |
...
| **ADR-039** | **Nexostrat bot runs as tenant in central Brain Bot Hub (shared infra, isolated data, separate token + API key)** | **Accepted** | 2026-05-21. After Session B of the Brain Bot Platform audit, Nexostrat accepts the tenant model: one Python process at `/srv/brain-hub/` runs all three bot identities (Personal/Mevillo/Nexostrat) under shared infrastructure (Whisper, Ollama, scheduler, events watcher). Strict isolation enforced at the tenant boundary via ScopedFS + manifest validator + scope-keyed event routing. Two binding amendments: (1) separate `NEXOSTRAT_BOT_TOKEN`, (2) Nexostrat-owned OpenAI API key with its own budget envelope (not the Brain `$10` global cap). Supersedes ADR-020. Full ADR body at [`00_GOVERNANCE/adr/ADR-039-bot-tenant-in-brain-hub.md`](../../00_GOVERNANCE/adr/ADR-039-bot-tenant-in-brain-hub.md). Cross-references the Brain Bot Platform design + audit at `/srv/brain/00_META/governance/plans/2026-05-21_brain_bot_platform_*.md` (Brain-side artifact; permitted reference per CLAUDE.md Strict Rule #4 clarification, since the citation is governance-traceability not infrastructure-coupling). |
