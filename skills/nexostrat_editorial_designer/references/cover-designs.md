...
Los logos están bundleados en `assets/logos/` dentro del directorio de este skill. Cuando generes código Python o JavaScript, construye la ruta a partir del directorio del skill — lo sabes porque leíste SKILL.md desde ahí.

```python
import os

# Sustituir con la ruta real al skill en el sistema actual
# (la sabes porque fue desde donde leíste SKILL.md)
SKILL_DIR = "/ruta/al/skill/nexostrat-editorial-designer"
LOGOS_DIR = os.path.join(SKILL_DIR, "assets", "logos")

# Lockups horizontales (1175×260px — ratio 4.52:1)
LOGO = {
...
# Íconos (430×500px — ratio 0.86:1, más alto que ancho)
ICONO = {
...
def draw_icono(c, icono_key, x, y_top, width=0.5*72):
    ...
    from reportlab.lib.utils import ImageReader
    aspect = 500 / 430    # ratio fijo del PNG (más alto que ancho)
    height = width * aspect
...
│                                         │
│ Propuesta de Consultoría en IA          │ ← y=42% | Century Gothic Bold 36pt Arctic White
│                                         │
│ Para: [Nombre de la Empresa]            │ ← y=52% | Century Gothic Regular 18pt Sky Blue
│                                         │
│                                         │
│                                         │
│ Crece sin contratar.                    │ ← y=82% | tagline itálica 12pt Arctic White
│ Escala sin complicarte.                 │
│ Mayo 2026  ·  Confidencial              │ ← y=88% | Century Gothic Regular 11pt Gray 500
│ nexostrat.com                           │ ← y=91%
└─────────────────────────────────────────┘
```
...
# Fuentes — Century Gothic con fallback a Helvetica
try:
    from reportlab.pdfbase import pdfmetrics
    from reportlab.pdfbase.ttfonts import TTFont
    pdfmetrics.registerFont(TTFont('CenturyGothic',     'Century Gothic.ttf'))
    pdfmetrics.registerFont(TTFont('CenturyGothic-Bold','Century Gothicb.ttf'))
    HF, HFB = 'CenturyGothic', 'CenturyGothic-Bold'
except:
...
// El logo se inserta con ImageRun — requiere leer el PNG desde assets/logos/
const fs = require('fs');
const path = require('path');

const SKILL_DIR = "/ruta/al/skill/nexostrat-editorial-designer"; // ajustar al path real
const logoPath = path.join(SKILL_DIR, "assets", "logos", "Nexostrat_Logo_Fondo_Midnight_Transparente.png");
const logoData = fs.readFileSync(logoPath);

// Logo PNG: 1175×260px → ancho 1.5" = 1371600 EMU → alto = 1371600 × (260/1175) = 303625 EMU
const LOGO_W = 1371600;
const LOGO_H = Math.round(LOGO_W * (260 / 1175));  // ≈ 303625 EMU
          ...
          children: [
            ...
            new Paragraph({
              children: [new TextRun({
                text: titulo, font: 'Century Gothic', size: 72, bold: true, color: 'F0FBFF'
              })],
            ...
            new Paragraph({
              children: [new TextRun({
                text: `Para: ${empresa}`, font: 'Century Gothic', size: 36, color: '0EA5E9'
              })],
            ...
            new Paragraph({
              children: [new TextRun({
                ...
                font: 'Century Gothic', size: 22, italic: true, color: 'F0FBFF'
              })],
            ...
            new Paragraph({
              children: [new TextRun({
                text: `${fecha}  ·  Confidencial`, font: 'Century Gothic', size: 22, color: '6B7280'
              })]
            }),
            new Paragraph({
              children: [new TextRun({ text: 'nexostrat.com', font: 'Century Gothic', size: 22, color: '6B7280' })]
            }),
          ]
        })
      ]
...
│                                         │
│                                         │
│ DIAGNÓSTICO INICIAL DE IA               │ ← y=40% | Century Gothic Regular 16pt Sky Blue
│ ─────────────────────────               │ ← y=44% | línea Sky Blue
│                                         │
│ [Nombre de la Empresa]                  │ ← y=48% | Century Gothic Bold 42pt Arctic White
│                                         │
│ Reporte de Oportunidades                │ ← y=58% | Century Gothic Regular 18pt Arctic
│                                         │
│                                         │
│ Crece sin contratar.                    │ ← y=82% | tagline itálica 12pt Arctic White
│ Escala sin complicarte.                 │
...
```python
# En docx-js — header con logo PNG
const headerLogoPath = path.join(SKILL_DIR, "assets", "logos",
                                  "Nexostrat_Logo_Fondo_Arctic_Transparente.png");
const headerLogoData = fs.readFileSync(headerLogoPath);

// Logo 1.5" ancho → 1371600 EMU; alto = 1371600 × (260/1175) ≈ 303625 EMU
const HDR_LOGO_W = 1371600;
const HDR_LOGO_H = Math.round(HDR_LOGO_W * (260 / 1175));
...
try:
    ...
    # Fallback a Helvetica si Century Gothic no está instalada en el sistema
    HF, HFB = 'Helvetica', 'Helvetica-Bold'
```
