...
// path requires: const path = require("path")
...
**Importante:** El script requiere que `assets/logos/` esté presente con los PNGs (18 archivos). La ruta se resuelve con `path.join(__dirname, "assets", "logos")` donde `__dirname` es el directorio desde el que se corre el script — asegurarse de que el directorio de trabajo sea la raíz del skill.

La fuente actual es **Inter**. Si la fuente cambia, hacer replace_all de `"Inter"` en el script.
